...
class Importer(csvreader.Importer, banking.Importer):
    ...
    def get_main_account(self, ot):
        ...
        if account_leaf:
            return self.config['main_account'].format(Name=account_leaf[ot.Name],
                                                      Claim_Type=self.claim_type_map(ot.Claim_Type))
        return self.config['main_account'].format(Claim_Type=self.claim_type_map(ot.Claim_Type))

    def get_target_account(self, ot):
        account_leaf = self.config.get('account_leaf', {})
        if account_leaf:
            return self.config['target_account'].format(Name=account_leaf[ot.Name],
                                                      Claim_Type=self.claim_type_map(ot.Claim_Type))
    ...
    def build_metadata(self, file, metatype=None, data={}):
        fa = {'filing_account': self.config['filing_account']} if 'filing_account' in self.config else {}
        ot = data['transaction']
